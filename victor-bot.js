...
// Routes to ChatGPT for writing/copy, Claude for everything else. Grok disabled.
...
victorBot.onText(/\/start/, (msg) => {
  ...
  victorBot.sendMessage(msg.chat.id,
    ...
    "🧠 *Claude* — Strategic analysis (default)\n" +
    "✍️ *ChatGPT* — Writing & content\n\n" +
    "I route automatically. Use /help for commands.",
    { parse_mode: "Markdown" }
  );
});
...
victorBot.onText(/\/brief_mimi (.+)/, async (msg, match) => {
  ...
  try {
    ...
    await handleVictor(chatId, "I just sent this directive to Mimi at Company C: \"" + match[1] + "\". Confirm what I expect back from her.");
...
console.log("Victor is online — Claude + ChatGPT only. Grok disabled.");
